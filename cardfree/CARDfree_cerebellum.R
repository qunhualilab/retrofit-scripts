...
library(DescTools)
library(corrplot)
marker <- "/storage/group/qul12/default/retrofit/Mouse/MouseBrain_pseudomarkers_new.csv"
df=read.csv(marker, row.names = 1)
loc <- "/storage/group/qul12/default/retrofit/Mouse/Cerebellum_coords.csv"
location <- read.csv(file = loc,row.names = 1)
row.names(location) <- gsub("-",".",row.names(location))
colnames(location)[1:2] <- c("x", "y")    
input_x = read.csv("/storage/group/qul12/default/retrofit/Mouse/Cerebellum_counts.csv")
rownames(input_x) = input_x[,1]
input_x = as.matrix(input_x[,-1])
# Create marker gene list: od gene + sc expression
gt_W0 = read.csv("/storage/group/qul12/default/retrofit/Mouse/Cerebellum_all_W.csv")
rownames(gt_W0) = gt_W0[,1]
gt_W0 = as.matrix(gt_W0[,-1])# [,c(1:5)]
de_gene = read.table("/storage/group/qul12/default/retrofit/Mouse/61mouse_od_gene.txt")
